...
parser = argparse.ArgumentParser(description="Nested cross-validation Random Forest regression")
...
print(f"Grid contains {total_combinations} parameter combinations ({fits_per_outer_fold} fits per outer fold)\n")
...
for file_idx, file in enumerate(files, 1):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X), 1):
        ...
        print(f"Starting inner CV ({fits_per_outer_fold} fits)...")
        ...
        print(f"Completed in {inner_cv_time:.2f} sec ({inner_cv_time/60:.2f} min)")
    ...
    results.append({
        ...
        "InnerCV_Time_mean_sec": fold_df["InnerCV_Time_sec"].mean(),
        "InnerCV_Time_ci_sec": 1.96 * fold_df["InnerCV_Time_sec"].std(ddof=1) / np.sqrt(n),
        "Dataset_Total_Time_sec": dataset_time
    })

    print(f"\n[DONE] {file}")
    print(f"Dataset runtime: {dataset_time:.2f} sec ({dataset_time/60:.2f} min)")
